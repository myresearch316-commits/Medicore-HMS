...
app.get('/api/health', async (req, res) => {
  const base = { ok: true, version: mem.version, feedback: feedbackMem.length };
  if (!pool) return res.json({ ...base, db: false, store: 'memory' });
  try {
    await pool.query('SELECT 1');
    res.json({ ...base, db: dbReady, store: dbReady ? 'postgres' : 'memory' });
  } catch (e) {
    res.json({ ...base, db: false, store: 'memory', error: String(e.message || e) });
  }
});
...
if (pool) {
  initDb()
    .then(() => start('MediCore HMS running (PostgreSQL)'))
    .catch((e) => {
      console.error('DB init failed, using in-memory/file store; will keep retrying:', e.message);
      start('MediCore HMS (no DB yet — file store)');
      // Auto-recover: keep retrying the database every 30s so it reconnects
      // automatically if the DB was temporarily down or comes back later.
      const retry = setInterval(() => {
        initDb()
          .then(() => { console.log('Database reconnected.'); clearInterval(retry); })
          .catch(() => {});
      }, 30000);
    });
} else {
